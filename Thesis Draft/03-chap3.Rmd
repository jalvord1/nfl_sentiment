...
In addition to the names and twitter handles, the race of the players is added to the data file, which was based entirely on our perceptions. 
...
Once the file is parsed, it is loaded into a new file for cleaning. Here, we use the function `bind_rows()` from the `dplyr` package [@dplyrpackage] to stack the two halves and create one data frame with 618,628 rows and 88 variables. These 88 variables each contain a piece of metadata that is provided by Twitter. Some variables include `user_id`, `created_at`, and `is_retweet`. The next step is to clean the text in order to pull out the names or twitter handles contained in the tweets in order to determine which players the tweet is mentioning.
...
```{r, warning=FALSE, message=FALSE, size = "small", include = FALSE}
...
We employ the function `str_extract_all()` from the `stringr` package [@stringrpackage] to test whether the text column contains any of the name or Twitter handles. A list-column is created because many of the tweets matche multiple names or twitter handles (when multiple players are mentioned in one tweet). Then, to create a data frame which duplicates the tweet for each player mentioned, we use the `unnest()` function from the `tidyr` package [@tidyrpackage]. This function takes a list-column then makes each element of the list its own row.
...
full_more_unnest %>%
  ...
  kable(caption = "Head of Unnestest Tweet Data Frame",
      caption.short = "Head of Unnestest Tweet Data Frame") %>%
...
tweet_words %>%
  ...
  kable(caption = "Head of Tweet Words Data Frame",
      caption.short = "Head of Tweet Words Data Frame") %>%
  kable_styling(bootstrap_options = "striped", 
                latex_options = "hold_position")
```
...
full_sentiments %>%
  ...
  kable(caption = "Head of Tweet Words with Sentiment Data Frame",
      caption.short = "Head of Tweet Words with Sentiment Data Frame") %>%
  kable_styling(bootstrap_options = "striped", 
                latex_options = "hold_position")
```
...
starters_sentiment %>%
  ...
  kable(caption = "Head of Starters Data Frame with Sentiment Percentages",
      caption.short = "Head of Starters Data Frame with Sentiment Percentages") %>%
...
In total, `r format(nrow(tweets_final), scientific=FALSE, big.mark = ",")` tweets are gathered for the 50 starting players during the 7-hour specified period. 
...
A few observations from tables \@ref(tab:tweetspatst) and \@ref(tab:tweetsramst) are that the player with the most tweets on both teams is the quarterback and a majority of the players in the top 5 are offensive players. In addition, we recognize that the distribution of tweets across both teams closely matches the Zipf's law, which states that the probability of attaining a certain $x$ is proportional to $x^{-t}$, where $t >= 1$. This law is commonly used in studies of word frequencies.
